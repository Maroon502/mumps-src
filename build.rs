...
pub fn download_mumps<P: AsRef<Path>>(download_dir: P, lib_version: &str) -> Result<PathBuf> {
    ...
    if !dest.exists() {
        let buf = get_agent().get(url.as_str()).call()?.into_body().into_reader();
...
fn Mumps_files_common() -> (Vec<String>, Vec<String>) {
    let files_common_mod = vec![
        ...
        "mumps_comm_buffer_common.F".to_string(),
        "mumps_intr_types_common.F".to_string(),
        ...
        "sol_omp_common_m.F".to_string(),
        "tools_common_m.F".to_string(),
    ];
...
fn Mumps_files_arith(arith: &str) -> (Vec<String>, Vec<String>) {
    ...
    let files_mod = vec![
        ...
        "mumps_intr_types.F".to_string(),
        "fac_asm_master_ELT_m.F".to_string(),
        "fac_asm_master_m.F".to_string(),
        "fac_compact_factors_m.F".to_string(),
